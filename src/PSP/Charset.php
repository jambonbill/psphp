...
class Charset
{
    ...
    /**
     * Convert charset into a lookup table
     * @return array [pixels]
     */

    private $_PIXELS=[];//The binary charset data in a lookup table
    ...
    public function pixels($charsetCase='uppercase')
    {
        ...
        if ($charsetCase=='lowercase') {
            $this->charset = imagecreatefromstring ( base64_decode($this->LOWER) );
        } else {
            $this->charset = imagecreatefromstring ( base64_decode($this->UPPER) );
        }
        ...
        for($col=0;$col<16;$col++){
            for($row=0;$row<16;$row++){
                ...
                for($y=0;$y<8;$y++){
                    for($x=0;$x<8;$x++){
                        $rgb=ImageColorAt($this->charset, $col*8+$x, $row*8+$y);
                        $r = ($rgb >> 16) & 0xFF;
                        $g = ($rgb >> 8) & 0xFF;
                        $b = $rgb & 0xFF;
                        $out=($r+$g+$b)/3;
